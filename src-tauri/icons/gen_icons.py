...
使用 Pillow 将 logo.png 生成为 Tauri 所需的全套 icon 文件
"""
...
SRC = "/Users/skygreen/Downloads/logo.png"
...
ICONS = {
    "32x32.png":      (32,  32),
    "128x128.png":    (128, 128),
    "128x128@2x.png": (256, 256),  # @2x = 2倍分辨率，实际尺寸256
    "icon.png":       (512, 512),  # 通用高清icon
}

# macOS .icns 需要的尺寸（可选，供参考）
...
def make_icns(img_path, out_path):
    ...
    for size, type_tag in icns_types.items():
        ...
        # icns element: type(4) + length(4, includes 8 bytes header) + data
        length = len(png_bytes) + 8
        icon_data += type_tag + struct.pack('>I', length) + png_bytes
    
    # icns header: 'icns' + total_length(4)

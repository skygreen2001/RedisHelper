...
pub fn connect(req: ConnectRequest) -> Result<bool, String> {
    ...
    // 使用连接池复用连接
    match RedisConnection::from_cache(&req.host, req.port, req.username.as_deref(), req.password.as_deref(), req.db) {
        Ok(mut conn) => {
...
pub fn get_databases(req: ConnectRequest) -> Result<Vec<(u8, usize)>, String> {
    ...
    // 使用连接池复用连接
    match RedisConnection::from_cache(&req.host, req.port, req.username.as_deref(), req.password.as_deref(), 0) {
        Ok(mut conn) => {
            let t1 = std::time::Instant::now();
...
pub fn get_key_value(req: KeyRequest) -> Result<KeyValueResponse, String> {
    match RedisConnection::new_with_auth(&req.host, req.port, req.username.clone(), req.password.clone()) {
        Ok(mut conn) => {

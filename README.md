...
- **v1.2.0** (2026-05-22)：新增多窗口多标签会话管理、浏览器兼容模式（Web平台支持）、Redis内存分析功能（内存仪表盘/键类型分布/大键排行）、Redis监控面板（服务器信息/内存状态/键统计）、Redis慢日志查看、服务器配置导入导出、中文原生菜单支持、键类型分布统计、可拖拽分隔条、键值详情刷新按钮；重构标签栏状态管理、服务器选择UI、后端接口参数格式统一；修复Redis SCAN游标未更新问题

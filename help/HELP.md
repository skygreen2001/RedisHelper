...
| 功能 | 说明 |
|------|------|
| 多服务器管理 | 添加、编辑、删除多个 Redis 服务器配置，支持 Redis 6.0+ ACL 用户名认证 |
| 数据库管理 | 切换数据库，新增/删除 DB |
| 键值操作 | 查看、添加、编辑、删除键值 |
| 数据类型 | 支持 String、List、Set、ZSet、Hash 五种类型 |
| 元素级编辑 | List/Set/ZSet/Hash 支持表格化元素编辑，可增删改查单个元素 |
| 搜索 | 按模式搜索键（支持 `*` 通配符） |
| 排序 | 按键名升序或降序排列，ZSet 支持按 Score/Member 排序 |
| 多选删除 | 批量选择键并删除废键箱 |
...
| 操作审计日志 | 记录所有键值读写操作，支持筛选、分页、统计分析，可全局开关 |
...
3. 填写服务器信息：
   ...
   - **用户名**：Redis ACL 用户名（可选，仅 Redis 6.0+ 需要填写）
   - **密码**：Redis 密码（如有）
   - **默认DB**：默认连接的数据库编号
4. 点击 **「测试连接」** 确认连接成功
5. 点击 **「确定」** 保存
...
### 4.7 元素级编辑（List/Set/ZSet/Hash）

对于复杂类型（List、Set、ZSet、Hash），除了原有的 JSON 覆盖编辑模式外，还支持**元素编辑模式**，可以像操作表格一样对单个元素进行增删改查。

#### 切换编辑模式

在值展示区顶部，复杂类型会显示模式切换按钮：

| 模式 | 说明 |
|------|------|
| **覆盖模式** | 原有方式，在 textarea 中编辑 JSON，整体替换值 |
| **元素编辑** | 表格形式展示，支持对单个元素增删改查 |

#### 元素编辑操作

| 操作 | 方式 |
|------|------|
| **修改元素** | 点击行内的编辑图标，修改后保存 |
| **删除元素** | 点击行内的删除图标 |
| **添加元素** | 点击「添加元素」按钮，弹出对话框输入 |
| **搜索过滤** | 在搜索框输入关键词过滤元素 |
...
| List | Value（值） |
| Set | Value（成员） |
| ZSet | Member（成员） + Score（分数） |
| Hash | Field（字段） + Value（值），重复 Field 会弹出覆盖确认 |

#### ZSet 排序

ZSet 元素编辑模式下，工具栏额外提供排序选项：
...
1. 在服务器设置页面（**「连接 ▼」** → **「设置」**）底部，可看到「操作审核」开关
2. 开启时（默认），所有键值读写操作将被记录到审计日志
3. 关闭后，审计日志不再记录新操作，但已记录的日志仍可查询
4. 开关状态持久化保存在配置文件中

> **注意**：此开关为全局配置，影响所有服务器的审计行为。
...
每次键值操作会增加一次 LPUSH 写入（<1ms），对正常使用几乎无感知。如不需要审计，可在服务器设置中关闭操作审核开关。

### Q: 什么是元素级编辑模式？

对于 List/Set/ZSet/Hash 类型的键，除了原有的 JSON 整体覆盖编辑方式外，还支持元素编辑模式。在元素编辑模式下，数据以表格形式展示，可以对单个元素进行增加、删除、修改操作，无需重写整个 JSON。在值展示区顶部切换「覆盖模式」和「元素编辑」即可。

### Q: 如何连接需要用户名认证的 Redis 服务器？

Redis 6.0+ 支持 ACL（访问控制列表），在添加或编辑服务器配置时，填写「用户名」字段即可。如果 Redis 未配置 ACL，用户名留空即可。

### Q: 支持通过 Docker 部署吗？

支持。项目提供了 Dockerfile、fly.toml、render.yaml 配置文件，可通过 Docker、Fly.io、Render 等平台部署 Web 版本。部署后通过浏览器访问，WebSocket 代理同时提供静态文件服务和 Redis 转发。
...
4. **数据格式**：List/Set/ZSet/Hash 类型的值使用 JSON 格式存储和展示（覆盖模式下）
5. **分页加载**：键列表采用分页加载机制，首次加载 100 个，可通过「加载所有」加载全部
6. **ACL 认证**：用户名字段仅适用于 Redis 6.0+ 的 ACL 功能，旧版 Redis 留空即可
7. **操作审核开关**：关闭操作审核后，新的键值操作不会被记录到审计日志
